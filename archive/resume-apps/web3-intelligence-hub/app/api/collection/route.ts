...
export async function POST(request: NextRequest) {
  try {
    ...
    // Fetch NFTs and collection metadata in parallel
    const [nftData, collectionData] = await Promise.all([
      fetchNftCollection(address, pageKey),
      pageKey ? Promise.resolve(null) : fetchCollectionMetadata(address),
    ])

    // Calculate rarity scores for the fetched NFTs
    const nftsWithRarity = calculateRarityScores(nftData.nfts)

    // Return response shape expected by useNFTCollection hook
    return NextResponse.json({
      nfts: nftsWithRarity,
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch NFT data'
